...
// (\:backslash)
    ...
    public partial class AALSTechGenEx: AnalyzerBaseV3{
		private int GStageMemo;
		private ALSLinkManEx fALS;

		public AALSTechGenEx( GNPX_AnalyzerManEx pAnMan ): base(pAnMan){
			fALS = new ALSLinkManEx(pAnMan);
        }
        ...
        public bool SueDeCoq( ){
            ...
            foreach( var ISPB in fALS.ALSLst.Where(p=> p.tfx>=18 && p.Size>=3) ){     //Selecte Block-type ALS
                ...
                foreach( var ISPR in fALS.ALSLst.Where(p=> p.tfx<18 && p.Size>=3) ){　//Selecte Row-type/Column-type ALS
                    ...
                    int IS_FreeB = IS.AggregateFreeB(pBDL);      //Intersection numbers
        ...
        public IEnumerable<UCellEx> _GetRestCells( UALSEx ISP, int selB ){
            return pBDL.IEGetCellInHouse(ISP.tfx,selB).Where(P=>!ISP.B81.IsHit(P.rc));
        }
        private void SuDoQueEx_SolResult( UALSEx ISPB, UALSEx ISPR ){
            Result="SueDeCoq";
        }
    }
}