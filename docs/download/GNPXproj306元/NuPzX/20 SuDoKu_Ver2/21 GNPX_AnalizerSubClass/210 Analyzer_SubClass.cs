...
#region Ex
        ...
        public int  AggregateFreeB( List<UCellEx> XLst ){
            return this.IEGet_rc().Aggregate(0,(Q,q)=>Q|XLst[q].FreeB);
        }
        ...
        static public bool operator==( Bit81 A, Bit81 B ){
            try{
                if( !(A is Bit81) || !(A is Bit81) )  return false;
        ...
        static public bool operator!=( Bit81 A, Bit81 B ){
            try{
                if( !(A is Bit81) || !(A is Bit81) )  return true;
...
*/
        ...
        public int GetBitPattern_rcN( int rc ){
            int bp=0;
            foreach( var n in nzBit.IEGet_BtoNo() ){
                if( _BQ[n].IsHit(rc) )  bp|=1<<n;
            }
            return bp;
        }
