    ...
    public partial class ALSTechGenEx: AnalyzerBaseV3{
        ...
        private bool _ALS_DeathBlossomSub( int sz ){
            foreach( var SC in pBDL.Where(p=>p.FreeBC==sz) ){ //Stem Cell
                List<LinkCellExAlsEx> LinkCeAlsLst=ALSMan.LinkCeAlsLst[SC.rc];
                ...
                while(cmb.Successor(nxt)){
                    ...
                    foreach( var no in AFreeB.IEGet_BtoNo() ){
                        ...
                        var LKCAsol=new List<LinkCellExAlsEx>();
        ...
        private void _DeathBlossom_SolResult( List<LinkCellExAlsEx> LKCAsol, UCellEx SC, int no, bool stmLink=false ){
