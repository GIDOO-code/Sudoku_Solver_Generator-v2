...
namespace GNPZ_sdk {
  #region UALSEx
    public class UALSEx: IComparable{
        ...
        public readonly List<UCellEx> UCellLst = new List<UCellEx>();    //ALS Cells
        ...
        public List<UALSPairEx>  ConnLst;
        public List<int>       LockedNoDir;
           
        public Bit81    rcbFilter;

        public UALSEx( int ID, int Size, int tfx, int FreeB, List<UCellEx> UCellLst ){
            this.ID        = ID;
            this.Size      = Size;
            this.tfx       = tfx;
            this.singly    = true;
        ...
        public UGrCellsEx SelectNoCells( int no ){
            int noB=1<<no;
            List<UCellEx> UCsS = UCellLst.FindAll(Q=>(Q.FreeB&noB)>0);
            UGrCellsEx GCs = new UGrCellsEx(tfx,no);
            GCs.Add(UCsS);
            return GCs;
        }
        ...
        public override string ToString(){
            string po = "<> UALSEx "+ID+" <>  tfx:"+tfx +" Size:"+Size +" Level:"+Level;
  ...
  //The following two classes will be developed collectively into GroupedLink (I wonder)
  #region UALSPairEx
    public class UALSPairEx{
        public readonly UALSEx ALSpre;
        public readonly UALSEx ALSnxt;
        public readonly int  RCC;
        public readonly int  nRCC=-1; //no:0...8(In the case of doubly, make links individually)
        public Bit81         rcUsed;
        public UALSPairEx( UALSEx ALSpre, UALSEx ALSnxt, int RCC, int nRCC, Bit81 rcB=null ){
            this.ALSpre=ALSpre; this.ALSnxt=ALSnxt; this.RCC=RCC; this.nRCC=nRCC;
            this.rcUsed = rcB?? (ALSpre.B81|ALSnxt.B81);
        }
        public  override bool Equals( object obj ){
            var A = obj as UALSPairEx;
  ...
  #endregion UALSPairEx

  #region LinkCellAlsEx
        public class LinkCellExAlsEx: IComparable{
        public readonly UCellEx UC;
        public readonly UALSEx  ALS;
        public readonly int   nRCC=-1; //no:0...8(In the case of doubly, make links individually)
        public LinkCellExAlsEx( UCellEx UC, UALSEx ALS, int nRCC ){
            this.UC=UC; this.ALS=ALS; this.nRCC=nRCC;
        }
        public  override bool Equals( object obj ){
            var A = obj as LinkCellExAlsEx;
            return (this.ALS.ID==A.ALS.ID);
        }
        public int CompareTo( object obj ){
            LinkCellExAlsEx A = obj as LinkCellExAlsEx;
            return (this.ALS.ID-A.ALS.ID);
        }

        public override string ToString(){
            string st="LinkCellAlsEx nRCC:"+nRCC.ToBitString(9);
            st += " UALSEx:"+UC+"\rALS:"+ ALS;
            return st;
        }
        public override int GetHashCode(){ return base.GetHashCode(); }
    }
  #endregion LinkCellAlsEx
}
