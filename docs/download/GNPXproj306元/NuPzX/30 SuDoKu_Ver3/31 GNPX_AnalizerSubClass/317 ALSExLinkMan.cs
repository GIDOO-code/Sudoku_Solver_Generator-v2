...
namespace GNPZ_sdk {
    public class ALSLinkManEx{
        ...
        public List<UALSEx>     ALSLst;

        public List<ALSExLinkEx>        AlsInnerLink;   //innerLink
        public List<LinkCellExAlsEx>[]  LinkCeAlsLst;   //Cell->ALS
        public bool                 ALS2ALS_Link;   //ALS ->ALS

        public ALSLinkManEx( GNPX_AnalyzerManEx pAnMan ){
            this.pAnMan = pAnMan;
		}
        ...
        public int PrepareALSLinkMan( int nPls ){ //QALS_Search
            ...
            ALSLst = new List<UALSEx>();
            List<int> singlyMan = new List<int>();
            for( int nn=1; nn<=nPls; nn++ ){
                for( int tf=0; tf<27; tf++ ){
                    List<UCellEx> Pcells=pBDL.IEGetCellInHouse(tf,0x1FF).ToList();
        ...
        public IEnumerable<UALSEx> IEGetCellInHouse( int lvl, int noB, Bit81 Area, int tfx=-1 ){
        ...
        //RCC
        public int  Get_AlsAlsRcc( UALSEx UA, UALSEx UB ){           
        ...
        public void QSearch_AlsInnerLink(){
            ...
            foreach( var Pals in ALSLst.Where(p=>(p.Size>=2&&p.singly)) ){
                ...
                while( prm.Successor(2) ){
                    ...
                    UGrCellsEx GS = Pals.SelectNoCells(noS);//ALS cell group with noS
                    UGrCellsEx GD = Pals.SelectNoCells(noD);//ALS cell group with noD
        ...
        //            WriteLine($"ALSExLink {P.UGCellsA.GCToString()} -> tfx:{P.tfx} {P.UGCellsB.GCToString()}" );
        //   //    }
        //    } );
        }
        private  void _SetGroupedLink( UALSEx P, UGrCellsEx GS, UGrCellsEx GD, int tfx ){
            ALSExLinkEx ALSLK= new ALSExLinkEx(P,GS,GD,tfx);
            if( AlsInnerLink==null ) AlsInnerLink=new List<ALSExLinkEx>();
            ...
            //WriteLine( $"ALSExLink {GS.GCToString()} -> tfx:{tfx} {GD.GCToString()}" );
        }
        ...
        public void QSearch_Cell2ALS_Link( ){
            ...
            LinkCeAlsLst = new List<LinkCellExAlsEx>[81];
            ...
            foreach( var PA in ALSLst.Where(P=>P.singly) ){
                foreach( var no in PA.FreeB.IEGet_BtoNo() ){
                    ...
                    foreach( var P in H.IEGetUCeNoB(pBDL,noB) ){
                        var Q = new LinkCellExAlsEx(P,PA,no);
                        if( LinkCeAlsLst[P.rc]==null )  LinkCeAlsLst[P.rc]=new List<LinkCellExAlsEx>();
        ...
        public void QSearch_ALS2ALS_Link( bool doubly ){
            ...
            while (cmb.Successor()){
                UALSEx UA = ALSLst[cmb.Index[0]];
                UALSEx UB = ALSLst[cmb.Index[1]];

                int RCC = Get_AlsAlsRcc( UA, UB );
                if( RCC==0 ) continue;
                if( !doubly && RCC.BitCount()!=1 ) continue;

                if( UA.ConnLst==null )  UA.ConnLst=new List<UALSPairEx>();
                if( UB.ConnLst==null )  UB.ConnLst=new List<UALSPairEx>();
                foreach( var no in RCC.IEGet_BtoNo() ){
                    UALSPairEx LKX=new UALSPairEx(UA,UB,RCC,no);
                    if( !UA.ConnLst.Contains(LKX) ) UA.ConnLst.Add(LKX);
                    LKX=new UALSPairEx(UB,UA,RCC,no);
    ...
    public class ALSExLinkEx: GroupedLinkEx, IComparable{
        private const int  S=1, W=2;
        public int         ID;
        public UALSEx        ALSbase=null;

        public ALSExLinkEx( UALSEx ALSbase, UGrCellsEx UGCellsA, UGrCellsEx UGCellsB, int tfx ){
        ...
        public new int CompareTo( object obj ){
            ALSExLinkEx Q = obj as ALSExLinkEx;
