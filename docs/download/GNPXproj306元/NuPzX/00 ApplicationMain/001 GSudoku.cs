...
namespace GNPZ_sdk{
    public class GNPXApp000{
        ...
        public SDK_Ctrl         SDKCntrlEx;           //Problem Generator
        public GNPZ_EnginEx     GNPX_EngEx;           //Analysis Engine 
        ...
        public List<UProblem>   SDKProbLst = new List<UProblem>();
        public UProblem         pGP{ get{ return GNPX_Eng.pGP; } }  //current problem is in GNPX_Eng
        public UProblem         pGP_DgtRecog=null;
        public int[]            SDK81;
        ...
        public GNPXApp000( NuPz_Win pGNP00win ){
            ...
            SDKRecgMan = new sdkFrameRecgV3Man(this,fName:fName);//MLtype,MidLSize described in parameter file
            
//q            GNPX_EngEx   = new GNPZ_EnginEx(this, new UProblemEx());
//q            SDK_Ctrl.pGNPX_EngEx = GNPX_EngEx;
    ...
    #region file I/O
        public int SDK_FileInput( string fName, bool prbIniFlag ){
            ...
            using( StreamReader SDKfile=new StreamReader(fName) ){
                ...
                while( (LRecord=SDKfile.ReadLine()) !=null ){
                    ...
                    if( eLst[0].Length>=81 ){
                        try{
                            if( nc>=3 && eLst[2]!=null ) difLvl=eLst[2].ToInt();
                            if( difLvl<=0 || difLvl>30 ) difLvl=1;
                            
                            if( nc>=4 && eLst[3]!=null ) name=eLst[3];
                            else name=pName;
        ...
        public void SDK_StringInput( string st ){
            st = st.Replace(".", "0");
            List<UCell> BDLa=_stringToBDL(st);
            SDKProbLst.Add(new UProblem(999999999,BDLa,"",0));
            int ID=0;
            SDKProbLst.ForEach(P=>P.ID=(ID++));
            CurrentPrbNo=(ID-1);
        }

        public UProblem SDK_ToUProblem( int[] SDK81, string name="", int difLvl=0, bool saveF=false ){
            string st="";
        ...
        public UProblem SDK_ToUProblem( string st, string name="", int difLvl=0, bool saveF=false ){
            ...
            var UP=new UProblem(999999999,B,name,difLvl);
        ...
        public void SDK_FileOutput( string fName, bool append, bool fType81, bool SolSort, bool SolSet, bool SolSet2 ){
            ...
            using( StreamWriter fpW=new StreamWriter(fName,append,Encoding.UTF8) ){
                foreach( var P in qry ){
                    ...
                    //===== Preparation =====
                    solMessage = "";
                    if(SolSet) solMessage = SetSolution(GP:P,SolSet2:SolSet2,SolAll:SolSet);//output Solution UProblem GP, bool SolSet2, bool SolAll=false
                    ...
                    if(fType81){　//Solution(tytpe:line)
                        ...
                        LRecord += " " + (P.ID+1)+" "+P.DifLevel.ToString() +" \""+ P.Name+ "\" ";
                        LRecord += " \"" + P.TimeStamp+ "\"";
                        ...
                        LRecord = (P.ID+1).ToString() + ", " + P.DifLevel.ToString() +", \""+ P.Name+"\"";
                        LRecord += ","+ solMessage;
                        fpW.WriteLine(LRecord);
        ...
        public void btnFavfileOutput( bool fType81=true, bool SolSet=false, bool SolSet2=false ){
            ...
            using( var fpW=new StreamWriter(fNameFav,true,Encoding.UTF8) ){
                if( fType81 ){
                    ...
                    LRecord=LRecord.Replace("0",".");
                    LRecord += " " + (pGP.ID+1) + " " + pGP.DifLevel.ToString()+ " \"" + pGP.Name+"\"";
                    if(SolSet&&SolSet2) LRecord += " \""+SetSolution(pGP,SolSet2:true,SolAll:true)+" \"";//解を出力
                    LRecord += " \"" + pGP.TimeStamp+ "\"";
                }
                else{
                    LRecord = pGP.ID.ToString()+", " + GNPX_Eng.pGP.DifLevel.ToString();
                    LRecord += " \"" + pGP.Name+"\", \"" + prbMessage + "\"";
                    fpW.WriteLine(LRecord);
